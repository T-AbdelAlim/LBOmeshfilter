...
if __name__ == '__main__':
    ...
    eigenvectors=[50,100,150]

    ref = pv.read(Path(data_path, 'sagittal_inst_001_CN.ply'))
    mesh = pv.read(Path(data_path, 'sagittal_inst_004_CN.ply'))


    ref_mesh = MeshHarmonics(Path(data_path, 'sagittal_inst_001_CN.ply'), n_vertices='default')
    # ref_mesh.plot_harmonics(EV_list=eigenvectors)
    # plt.show()

    ## calculate basis functions and natural frequencies:
    mesh = MeshHarmonics(Path(data_path, 'sagittal_inst_004_CN.ply'), n_vertices='default')
